...
describe('formatCurrency', () => {
  ...
  it('renders rial when the profile unit is rial', () => {
    const rial = { profile: { ...LOCALES.fa, currency: { code: 'IRR', unit: 'rial' as const, divisor: 1 } } };
...
describe('formatDate / formatDateTime / formatTime', () => {
  ...
  it('can override calendar, numerals and time zone per call', () => {
    ...
    expect(formatDate(NOWRUZ, { ...fa, calendar: 'gregory', numerals: 'latn' })).toBe('21 مارس 2025');
...
describe('compare / sortBy', () => {
  ...
  it('sorts numerically by default', () => {
    expect(sortBy(['item10', 'item2', 'item1'], (s) => s, en)).toEqual(['item1', 'item2', 'item10']);
