...
// `start`/`end` are logical: in RTL `start` is the right edge. The slide
// animations are physical, so they are flipped under the rtl variant.
    ...
    variants: {
      side: {
        ...
        start:
          ...
          "inset-y-0 end-0 h-full w-3/4 border-s sm:max-w-sm data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right rtl:data-[state=closed]:slide-out-to-left rtl:data-[state=open]:slide-in-from-left",
      },
    },
    defaultVariants: {
      side: "end",
...
interface SheetContentProps
  ...
  /** Screen-reader text for the close button; defaults to the localized "Close". */
...
const SheetContent = React.forwardRef<
  ...
  SheetContentProps
>(({ side = "end", className, children, closeLabel = t("ui.close"), ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay />
