    ...
    <GregorianDayPickerBase
      ...
      formatters={{
        ...
        formatWeekdayName: (date, options, dateLib) =>
          (dateLib ?? defaultDateLib).format(date, locale.lang === "fa" ? "ccccc" : "cccccc", options),
        ...formatters,
      }}
      {...props}
    />
  ...
  () =>
    import("react-day-picker/persian").then(({ DayPicker, faIR, enUS }) => {
      function Jalali({ locale: localeProp, ...props }: DayPickerProps) {
        const jalaliLocale = (localeProp as DayPickerLocale | undefined) ?? (locale.lang === "fa" ? faIR : enUS)
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      modifiers={{
        weekend: (date) => locale.weekend.includes(date.getDay() as (typeof locale.weekend)[number]),
        ...modifiers,
      }}
      modifiersClassNames={{
