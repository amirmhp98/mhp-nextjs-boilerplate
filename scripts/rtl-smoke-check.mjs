...
function checkFile(filePath) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!isUiPrimitive && physicalSidePropRegex.test(raw)) {
      addViolation(filePath, lineNumber, `Physical side prop is forbidden. Use side="start" or side="end".`);
    }
...
function checkLayoutDefaults() {
  ...
  if (!layoutContent.includes('lang={locale.lang}')) {
    violations.push('src/app/layout.tsx:1 Root <html> must use lang={locale.lang} from @/lib/locale.');
  }
  if (!layoutContent.includes('dir={locale.dir}')) {
    violations.push('src/app/layout.tsx:1 Root <html> must use dir={locale.dir} from @/lib/locale.');
  }
