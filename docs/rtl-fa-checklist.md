...
این پروژه به‌صورت پیش‌فرض فارسی (`fa`)، راست‌به‌چپ (`rtl`) و با تقویم جلالی است و با تغییر یک متغیر (`NEXT_PUBLIC_LOCALE=en`) انگلیسی، چپ‌به‌راست و میلادی می‌شود. هیچ‌جای اپ نباید خودش تصمیم بگیرد که «الان RTL هستیم یا نه»؛ همه از پروفایل لوکال می‌خوانند.
...
- `NEXT_PUBLIC_LOCALE` در زمان build خوانده می‌شود؛ بعد از تغییر آن `npm run dev` را دوباره اجرا کنید یا build بگیرید.
...
- هر متن قابل‌مشاهده برای کاربر از `t('key')` / `tp('key', count)` در `@/lib/t` خوانده می‌شود و در `src/messages/{fa,en}.ts` تعریف می‌شود.
...
- `src/app/components/page.tsx` (مسیر `/components`) سطح مرجع رگرسیون بصری برای هر دو جهت است؛ هر کامپوننت جدید باید آنجا نمونه داشته باشد و یک بار با `NEXT_PUBLIC_LOCALE=en` هم دیده شود.
